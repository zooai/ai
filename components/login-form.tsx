...
import { createUserWithEmailAndPassword, signInWithEmailAndPassword, updateProfile, GoogleAuthProvider, signInWithPopup, FacebookAuthProvider, GithubAuthProvider } from 'firebase/auth'
...
export function LoginForm({ className, action = 'sign-in', ...props }: LoginFormProps) {
  ...
  const [formState, setFormState] = React.useState<{ first_name: string, last_name: string, email: string, password: string }>
    (
      {
        first_name: '',
        last_name: '',
  ...
  const signIn = async (e: any) => {
    e.preventDefault()
    try {
      const credentials = await signInWithEmailAndPassword(auth, formState.email, formState.password);
  ...
  const signUp = async (e: any) => {
    e.preventDefault()
    try {
      const credentials = await createUserWithEmailAndPassword(auth, formState.email, formState.password);
  ...
  return (
    <div {...props}>
        ...
        <fieldset className="flex flex-col gap-y-4">
          {
            action == 'sign-up' ?
              ...
              <div className='flex flex-col gap-y-4'><div className="flex flex-col gap-y-1">
      ...
      <div className='flex items-center justify-between gap-8'>
        ...
        <div onClick={facebookSignin} className='border border-[#8a8a8a] rounded-lg gap-4 flex items-center justify-center w-full py-2 cursor-pointer hover:bg-[#8787F7] '>
          <FaFacebook />

        </div>
        <div onClick={GithubSignin} className='border border-[#8a8a8a] rounded-lg gap-4 flex items-center justify-center w-full py-2 cursor-pointer hover:bg-[#8787F7] '>
          <FaGithub />
