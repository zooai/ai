...
function Subscription(props: any) {
  ...
  const subscriptionTeams = async () => {
    const { data } = await axios.get(
      `/api/subscription?code=${process.env.NEXT_PUBLIC_TEAMS_PRODUCT}`
    )
    await stripe!.redirectToCheckout({ sessionId: data })
  }
  const subscriptionCreator = async () => {
    const { data } = await axios.get(
      `/api/subscription?code=${process.env.NEXT_PUBLIC_CREATOR_PRODUCT}`
    )
    await stripe!.redirectToCheckout({ sessionId: data })
  }
  return (
      ...
      <div className="flex flex-col h-screen">
        ...
        <div className="flex flex-col overflow-auto">
          ...
          <div className="flex flex-col px-5 pt-6 space-y-3">
            ...
            <div
              className="border text-center w-full py-2 cursor-pointer hover:bg-white hover:text-black"
              onClick={subscriptionTeams}
            >
          ...
          <div className="flex flex-col px-5 pt-6 space-y-3">
            ...
            <p className="text-xs">
              Powerful AI features to create & improve your content everywhere
              you work online.
            </p>
            <div
              className="border text-center w-full py-2 cursor-pointer hover:bg-white hover:text-black"
              onClick={subscriptionCreator}
            >
              Start free 7-day trial
            </div>
          </div>
          <div className="flex flex-col px-5 py-6 space-y-3">
            ...
            <p className="text-xs">
              Personalized AI features with additional control, security, team
